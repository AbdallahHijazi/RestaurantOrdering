...
export interface TablesSelectOption {
  value: string;
  label: string;
}

@Component({
  selector: 'app-tables-management-select',
  templateUrl: './tables-management-select.html',
  styleUrl: './tables-management-select.scss',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TablesManagementSelect),
      multi: true,
    },
  ],
})
export class TablesManagementSelect implements ControlValueAccessor {
  private readonly platformId = inject(PLATFORM_ID);
  private readonly hostRef = inject<ElementRef<HTMLElement>>(ElementRef);

  readonly options = input.required<TablesSelectOption[]>();
  ...
  protected listboxId(): string {
    return `${this.testId() ?? 'tables-select'}-listbox`;
  }
  ...
  protected selectOption(option: TablesSelectOption, index: number): void {
  ...
  protected onOptionKeydown(event: KeyboardEvent, option: TablesSelectOption, index: number): void {
    switch (event.key) {
      case 'ArrowDown':
        event.preventDefault();
        this.moveActive(1);
